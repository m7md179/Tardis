...
function makeDeps(config: Config): TransportDeps {
  return {
    ...
    fetchImpl: (url, init) =>
      fetch(url, {
        ...init,
        signal: AbortSignal.timeout(resolveTimeoutMs(config.requestTimeoutMs)),
      }),
    readToken: async () => {
      try {
...
async function runTime(
  ...
  console.log(
    `${date}: ${commits.length} commits, ${sessions.length} sessions, ${hhmm(worked)} worked`
  );
  ...
  const result = await invokeSkill(deps, 'workspace.log-branch-time', {
    date,
    allocations,
    replace,
  });
