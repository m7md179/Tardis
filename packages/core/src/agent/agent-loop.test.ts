...
//
// "give me 10 healthy low calorie recipes" put the question into the food diary
// — the selector picked `health` because the message said calories, and the
// model reached for the most prominent skill there. On another run the same
// tunnel produced the opposite failure: "I do not have a database of recipes. I
// can only assist with logging, summarizing, or deleting your health entries."

describe('buildSystemPrompt: asking vs telling', () => {
  const WEB: ToolDefinition = {
    name: 'web.search',
  ...
  it('tells the model a question records nothing', () => {
    const prompt = buildSystemPrompt(makeInput({ availableTools: [HEALTH] }));
    expect(prompt).toContain('Asking is not recording');
    expect(prompt).toContain('record nothing');
  });

  it('rules out declining for lack of a tool', () => {
    const prompt = buildSystemPrompt(makeInput({ availableTools: [HEALTH] }));
    expect(prompt).toContain('Never decline because no tool covers a question');
  });

  it('points at web.search only when the plugin is actually loaded', () => {
    // Naming a tool the model has not been given invites a call it cannot make.
    const withWeb = buildSystemPrompt(makeInput({ availableTools: [HEALTH, WEB] }));
    const withoutWeb = buildSystemPrompt(makeInput({ availableTools: [HEALTH] }));
    expect(withWeb).toContain('web.search');
    expect(withoutWeb).not.toContain('web.search');
  });

  it('keeps behaviour before wording — the ordering that is load-bearing', () => {
    const prompt = buildSystemPrompt(makeInput({ availableTools: [HEALTH, WEB] }));
    expect(prompt.indexOf('Asking is not recording')).toBeLessThan(
