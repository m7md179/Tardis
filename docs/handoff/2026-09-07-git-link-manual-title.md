...
export function gitLinkFallbackLabel(link) {
  return link.kind === 'pull_request' ? `${link.repoFullName}#${link.number}` : link.repoFullName; // branch AND commit both land here
}
```
...
> _"the manual route (which is how a branch-created item gets its link) never
> sets it at all"_
